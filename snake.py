...
class Snake:
    ...
    def move(self, dir):
        ...
        if(dir==0):
            if(self.snekTiles[0][1]==0):
                settimer(0)
            else: self.snekTiles[0]=[self.snekTiles[0][0],(self.snekTiles[0][1]-1)]
        if(dir==1):
            if (self.snekTiles[0][0]==9):
                settimer(0)
            else: self.snekTiles[0]=[(self.snekTiles[0][0]+1),self.snekTiles[0][1]]
        if(dir==2):
            if (self.snekTiles[0][1]==9):
                settimer(0)
            else: self.snekTiles[0]=[self.snekTiles[0][0],(self.snekTiles[0][1]+1)]
        if(dir==3):
            if (self.snekTiles[0][0]==0):
                settimer(0)
            else:
                self.snekTiles[0]=[(self.snekTiles[0][0]-1),self.snekTiles[0][1]]
...
while not done:
    ...
    if Snek.headY()!=0:
        if tiles[Snek.headX()][Snek.headY() - 1]==0:
            netInput.append(1)
        else: netInput.append(0)
    else: netInput.append(0)
    if Snek.headX()!=9:
        if tiles[Snek.headX() + 1][Snek.headY()]==0:
            netInput.append(1)
        else: netInput.append(0)
    else: netInput.append(0)
    if Snek.headY()!=9:
    ...
    else: netInput.append(1)
    if Snek.headX()!=0:
        if tiles[Snek.headX() - 1][Snek.headY()]==0:
            netInput.append(1)
        else: netInput.append(0)
    else: netInput.append(0)
    if Snek.currentGoalLoc[1]<Snek.headY():
        netInput.append(1)
    else: netInput.append(0)
    if Snek.currentGoalLoc[0]>Snek.headX():
        netInput.append(1)
    else: netInput.append(0)
    if Snek.currentGoalLoc[1]>Snek.headY():
        netInput.append(1)
